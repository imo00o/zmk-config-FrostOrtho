...
## 組み立て方法
### ケースから基板を取り出す
ケースには一切ネジを使用しておらず、スナップフィット機構にて固定しています。  
...
また、一列だけキースイッチの向きが逆になっているためご注意ください。  
...
&trackball_listener {
    scroller_y {
        ...
        input-processors = <
            ...
            &zip_scroll_y_scaler 3 3 // Y軸スクロール量3/3倍
        >;
    };
    scroller_x {
        layers = <7>; // 横スクロールレイヤー
        input-processors = <
            &zip_scroll_transform (INPUT_TRANSFORM_X_INVERT) // スクロール方向調整
            &zip_xy_to_scroll_mapper
            &zip_scroll_x_scaler 3 3 // X軸スクロール量3/3倍
